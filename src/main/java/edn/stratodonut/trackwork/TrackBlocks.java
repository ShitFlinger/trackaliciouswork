...
import edn.stratodonut.trackwork.tracks.blocks.variants.*;
...
//                            .getExistingFile(p.modLoc("block/" + c.getName() + "/" + suffix))).generate(c, p))
    ...
    public static final BlockEntry<TinySimpleWheelBlock> TINY_WHEEL =
            REGISTRATE.block("tiny_wheel", TinySimpleWheelBlock::new)
                    .initialProperties(() -> Blocks.RAIL)
...
//                    .onRegister(CreateRegistrate.blockModel(() -> BracketedKineticBlockModel::new))
    ...
    public static final BlockEntry<MediumSimpleWheelBlock> MEDIUM_WHEEL =
            REGISTRATE.block("medium_wheel", MediumSimpleWheelBlock::new)
                    .initialProperties(() -> Blocks.RAIL)
                    .properties(p -> p.noOcclusion().mapColor(MapColor.PODZOL).noCollission().strength(7.0f).sound(SoundType.COPPER))
                    .transform(BlockStressDefaults.setNoImpact())
                    .transform(pickaxeOnly())
...
//                    .onRegister(CreateRegistrate.blockModel(() -> BracketedKineticBlockModel::new))
    ...
    public static final BlockEntry<WagonSimpleWheelBlock> WAGON_WHEEL =
            REGISTRATE.block("wagon_wheel", WagonSimpleWheelBlock::new)
                    .initialProperties(() -> Blocks.RAIL)
                    .properties(p -> p.noOcclusion().mapColor(MapColor.PODZOL).noCollission().strength(7.0f).sound(SoundType.COPPER))
                    .transform(BlockStressDefaults.setNoImpact())
                    .transform(pickaxeOnly())
...
//                    .onRegister(CreateRegistrate.blockModel(() -> BracketedKineticBlockModel::new))
    ...
    public static final BlockEntry<? extends RotatedPillarBlock> MEDIUM_SIMPLE_WHEEL_PART =
            REGISTRATE.block("medium_simple_wheel_part", (properties) -> new RotatedPillarBlock(properties) {
                        @Override
                        public @NotNull VoxelShape getCollisionShape(BlockState state, BlockGetter worldIn, BlockPos pos, CollisionContext context) {
                            return AllShapes.CRUSHING_WHEEL_COLLISION_SHAPE;
                        }
