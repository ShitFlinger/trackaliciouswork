...
//        float yRot = (trackAxis == Direction.Axis.X) ? 0 : 90;
        ...
        if(be.getWheelRadius() > 0.5 || !be.isWooden) {
...
//        19.5
        ...
        float springBaseLength = 1/16f;
        float springEndPointLength = 11/16f;
        ...
        if(be.getWheelRadius() > 0.5 || !be.isWooden) {
            ...
            springBase.translate((springFlip ? 0f : 0) * -axisMult, 0, 0)
                    .unCentre();
            springBase.renderInto(ms, buffer.getBuffer(RenderType.cutout()));
        ...
        }
        {
            ...
            SuperByteBuffer wheels = be.getWheelRadius() < 0.5f
                    ? CachedBufferer.partial(trackworkPartialModels.TINY_SIMPLE_WHEEL, state) :
                    be.getWheelRadius() < 0.63f
                            ? CachedBufferer.partial(trackworkPartialModels.SIMPLE_WHEEL, state) :
                            be.getWheelRadius() > 1.0f
                                    ? CachedBufferer.partial(trackworkPartialModels.WAGON_WHEEL, state) :
                                    be.getWheelRadius() > 0.8f
                                            ? CachedBufferer.partial(trackworkPartialModels.LARGE_SIMPLE_WHEEL, state) :
                                            CachedBufferer.partial(trackworkPartialModels.MEDIUM_SIMPLE_WHEEL, state);
