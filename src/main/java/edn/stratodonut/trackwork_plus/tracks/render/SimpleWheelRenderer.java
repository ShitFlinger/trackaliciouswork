...
//        float yRot = (trackAxis == Direction.Axis.X) ? 0 : 90;
        ...
        ribTransform(CachedBufferer.partial(trackwork_plusPartialModels.SIMPLE_WHEEL_RIB, state), yRot, wheelTravel, ms, buffer, light,
                new Vec3(horizontalOffset * -axisMult, 0, 0), springFlip);
        ribTransform(CachedBufferer.partial(trackwork_plusPartialModels.SIMPLE_WHEEL_RIB_UPPER, state), yRot, wheelTravel, ms, buffer, light,
...
//        19.5
        ...
        SuperByteBuffer springBase = CachedBufferer.partial(trackwork_plusPartialModels.SIMPLE_WHEEL_SPRING_BASE, state);
...
//        float springScale = (0.6f * wheelTravel + 1f) / (13.5f/16);
        SuperByteBuffer springCoil = CachedBufferer.partial(trackwork_plusPartialModels.SIMPLE_WHEEL_SPRING_COIL, state);
...

        {
            ...
            SuperByteBuffer wheels = be.getWheelRadius() < 0.63f
                    ? CachedBufferer.partial(trackwork_plusPartialModels.SIMPLE_WHEEL, state) :
                    be.getWheelRadius() > 0.8f ? CachedBufferer.partial(trackwork_plusPartialModels.LARGE_SIMPLE_WHEEL, state) :
                            CachedBufferer.partial(trackwork_plusPartialModels.MED_SUSPENSION_WHEEL, state);
            wheels.centre()
...
//                    .translate(0, 8 / 16f, 0)
    ...
    public int getViewDistance() {
        return trackwork_plusConfigs.client().trackRenderDist.get();
    }
}
